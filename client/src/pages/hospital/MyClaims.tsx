...
const MyClaims = () => {
    ...
    useEffect(() => {
        const fetchClaims = async () => {
            setLoading(true);
            const data = await getHospitalClaims();
            setClaims(data);
            setLoading(false);
        };
        ...
        <div className="space-y-6 animate-fadeIn">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-xs text-slate-400 mt-1">
                        Track, filter, and inspect insurance claims logged by your hospital.
                    </p>
                </div>
                <Link
                    to="/hospital/submit-claim"
                    className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-500 text-slate-950 font-bold text-xs shadow-md shadow-emerald-500/20"
                >
            ...
            <div className="bg-slate-900/60 border border-slate-800/80 rounded-2xl p-4 backdrop-blur-xl flex flex-col md:flex-row items-center justify-between gap-4">
                <div className="relative w-full md:w-80">
                    ...
                    <input
                        type="text"
                        placeholder="Search by ID, patient, treatment..."
                        value={searchTerm}
                        onChange={(e) => setSearchTerm(e.target.value)}
                        className="w-full py-2 pl-9 pr-3 bg-slate-950/80 border border-slate-800 rounded-xl text-xs text-slate-100 placeholder-slate-500 focus:outline-none focus:border-emerald-500 transition-colors"
                    />
                </div>
                ...
                <div className="flex items-center gap-2 w-full md:w-auto">
                    ...
                    <select
                        ...
                        className="py-2 px-3 bg-slate-950/80 border border-slate-800 rounded-xl text-xs text-slate-100 focus:outline-none focus:border-emerald-500"
                    >
                        ...
                        <option value="more_information_required">Info Required ({claims.filter(c => c.status === "more_information_required").length})</option>
                    </select>
                </div>
            </div>
                    ...
                    <div className="py-16 text-center text-slate-400 space-y-3">
                        ...
                        <p className="text-xs text-slate-500 max-w-sm mx-auto">
                            {searchTerm || statusFilter !== "all"
                                ...
                                : "Your hospital has not submitted any claims yet."}
                        </p>
                    </div>
                ) : (
